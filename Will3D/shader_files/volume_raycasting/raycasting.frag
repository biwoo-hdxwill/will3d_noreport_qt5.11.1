...
//cutting parameters
uniform int CutStep;
uniform mat4 MapVolToMask;
uniform isampler3D CutMaskTex;
...
void cuttingRayCasting()
{
	...
	if (entryPoint.w < exitPoint.w && dir_len.w > StepSize)
	{
		...
		bool is_seg = false;
		vec3 seg_start_point;
		float seg_len, seg_prev_len;
		seg_prev_len = 0.0f;
		...
		for (; lengthAccum < len; lengthAccum += deltaX)
		{
			int mask_value = texture(CutMaskTex, vec3(MapVolToMask*vec4(currPoint, 1.0))).r;
			if (mask_value != 0)
			{
				mask_value = mask_value & 0x0001 << CutStep;
			}
			if (mask_value == 0)
			{
				if (!is_seg)
				{
					seg_start_point = currPoint;
					is_seg = true;
				}
			...
			else
			{
				if (is_seg)
				{
					vec4 seg_dir_len = vec4(dir_len.xyz, seg_len);

					if (isMIP)
						accumColor = max(mipRaycasting(seg_start_point, seg_dir_len, deltaX), accumColor);
					else if (isXRAY)
						accumColor = (accumColor*seg_prev_len + XRaycasting(seg_start_point, seg_dir_len, deltaX)*seg_len) / (seg_prev_len + seg_len);
					else if (isShade)
						accumColor = accumColor + (1.0 - accumColor.a)*shadeRaycasting(seg_start_point, seg_dir_len, deltaX);
					else
						accumColor = accumColor + (1.0 - accumColor.a)*raycasting(seg_start_point, seg_dir_len, deltaX);

					is_seg = false;
					seg_prev_len = seg_len;
					seg_len = 0.0f;
				}
			}
			currPoint += deltaDir;
