...
#include <queue>
#include "Inc/VpPathTracking.h"
#include "Inc/VpImageAPI.inl"
#include <float.h>

////////////////////////////////////////////////////////////////////////////////////////////////////////////
CVpPathTracking::CVpPathTracking(void)
	: m_nWidth(0), m_nHeight(0), m_nWH(0), m_nDepth(0), m_ppVolume(NULL), m_nEdge(0), m_nNode(0), m_nScale(0) {
	m_nStartX = 0;
	m_nStartY = 0;
	m_nStartZ = 0;
...
// 초기화 함수
bool CVpPathTracking::Init(int w, int h, int d, usvoltype** ppVolume, EMethod emethod, int kernelsize) {
	m_nWidth = w;
	m_nHeight = h;
	m_nWH = w * h;
	m_nDepth = d;
...
// 시드 설정 함수
void CVpPathTracking::SetPoints(int startx, int starty, int startz, int endx, int endy, int endz) {
	m_nStartX = startx;	m_nStartY = starty;	m_nStartZ = startz;
	m_nEndX = endx;	m_nEndY = endy;	m_nEndZ = endz;
}

// DIJKSTRA 실행
float CVpPathTracking::Do(unsigned char** ppOutMask) {
	float distance = .0f;
	switch (m_eMethod) {
	case DIJKSTRA:
...
// DIJKSTRA 실행
// pvecPathxy : path의 x, y 좌표 vector에 저장 (x, y index로 저장. 즉 x, y는 x + y*width 로 저장)
// pvecPathz : path의 z 좌표 vector에 저장
float CVpPathTracking::Do(std::vector<int>* pvecPathxy, std::vector<int>* pvecPathz) {
	float distance = .0f;
	switch (m_eMethod) {
	case DIJKSTRA:
		// 실제로 DIJKSTRA 실행
...
float CVpPathTracking::DoDijkstra(unsigned char** ppOutMask) {
...
float CVpPathTracking::DoDijkstra(std::vector<int>* pvecPathxy, std::vector<int>* pvecPathz) {
	...
	neighbordist[4].x = 1;	neighbordist[4].y = 0;	neighbordist[4].z = -1;

	neighbordist[5].x = -1;	neighbordist[5].y = 1;	neighbordist[5].z = -1;
	neighbordist[6].x = 0;	neighbordist[6].y = 1;	neighbordist[6].z = -1;
	neighbordist[7].x = 1;	neighbordist[7].y = 1;	neighbordist[7].z = -1;

	neighbordist[8].x = -1;	neighbordist[8].y = -1;	neighbordist[8].z = 0;
	neighbordist[9].x = 0;	neighbordist[9].y = -1; neighbordist[9].z = 0;
	neighbordist[10].x = 1;	neighbordist[10].y = -1; neighbordist[10].z = 0;

	neighbordist[11].x = -1; neighbordist[11].y = 0;	neighbordist[11].z = 0;
	neighbordist[12].x = 1;	neighbordist[12].y = 0;	neighbordist[12].z = 0;

	neighbordist[13].x = -1; neighbordist[13].y = 1;	neighbordist[13].z = 0;
	neighbordist[14].x = 0;	neighbordist[14].y = 1;	neighbordist[14].z = 0;
	neighbordist[15].x = 1;	neighbordist[15].y = 1;	neighbordist[15].z = 0;

	neighbordist[16].x = -1; neighbordist[16].y = -1; neighbordist[16].z = 1;
	neighbordist[17].x = 0;	neighbordist[17].y = -1; neighbordist[17].z = 1;
	neighbordist[18].x = 1;	neighbordist[18].y = -1; neighbordist[18].z = 1;

	neighbordist[19].x = -1; neighbordist[19].y = 0;	neighbordist[19].z = 1;
	neighbordist[20].x = 1;	neighbordist[20].y = 0;	neighbordist[20].z = 1;

	neighbordist[21].x = -1; neighbordist[21].y = 1;	neighbordist[21].z = 1;
	neighbordist[22].x = 0;	neighbordist[22].y = 1;	neighbordist[22].z = 1;
	neighbordist[23].x = 1;	neighbordist[23].y = 1;	neighbordist[23].z = 1;

	// 각 이웃까지의 노말벡터 미리 저장. 나중에 현재 복셀과 이웃까지의 direction 구할때 사용
	TPoint3D<float> neighnor[24];
	...
	// 실제로 DISTINTENSITY, DISTANCE, DIRECTION 세가지 feature만 사용
...
bool CVpPathTracking::Curve(std::vector<int>* pvecPathxy, std::vector<int>* pvecPathz, int sampling, float offsett, ECurve eCurve) {
	return Curve(m_nWidth, pvecPathxy, pvecPathz, sampling, offsett, eCurve);
}

bool CVpPathTracking::Curve(int w, std::vector<int>* pvecPathxy, std::vector<int>* pvecPathz, int sampling, float offsett, ECurve eCurve) {
	bool bRet = false;
	switch (eCurve) {
	case CRCURVE:
...
bool CVpPathTracking::Curve_CRCurve(int w, std::vector<int>* pvecPathxy, std::vector<int>* pvecPathz, int sampling, float offsett) {
	// 1. sampling path
